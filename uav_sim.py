...
# print("if this doesn't print I'm dead")
...
class UAVSim():
    ...
    def init_teleop(self):
        ...
        print("hello, initialising PyGame")
        pygame.init()
        # print("what happened aaa")
        SURFACE_WIDTH = 640
        SURFACE_HEIGHT = 200
    ...
    def set_follow_mode(self):
        self.follow_mode_current += 1

    def get_follow_mode(self):
        return self.follow_mode_current

    def get_mode(self):
        return self.mode_new

    def automatic_control(self,of,mode):
        # self.vx_c = 1
        # self.speed_val = 1
        # print(of)
        # self.speed_val = 0.5
        ...
        if mode==1:
            ...
            v = vel[2]
            # print("optical flow:", of," err from desired:",of-0.5,"z vel",v,"err compensated",of-0.5-v," alt:",self.alt_c_auto)
            self.alt_c_auto +=  0.05*(of - 0.5 - v)
            print(self.alt_c_auto)
        if mode==2:
            vel = self.get_body_velocity()
            v = vel[1]
            v2 = vel[2]
            ...
            of_r = of[1]
            self.alt_c = 5.0
            self.roll_c = 20*(abs(of_l[0])-abs(of_r[0]) - 2*v)#*np.cos(math.radians(self.roll_c)) - 2*v2*np.sin(math.radians(self.roll_c)))
            self.vy_c = self.roll_c/45
            self.speed_val = 0.5
            # print("optical flow:", of_l[0], of_r[0]," err from desired:",of_l[0]+of_r[0],"y vel",v,"err compensated",of_l[0]+of_r[0]-2*v," roll:",self.roll_c)#\
            #    2*v*np.cos(math.radians(self.roll_c)) - 2*v2*np.sin(math.radians(self.roll_c))," roll:",self.roll_c)# ,"z vel",v2,"err compensated",of_l[0]+of_r[0]- \
            #self.roll_c = 40*(abs(of_l[0])-abs(of_r[0]) - 2*v*np.cos(math.radians(self.roll_c)) - 2*v2*np.sin(math.radians(self.roll_c)))
            #self.vy_c = self.roll_c/45
            #self.speed_val = 0.5
            #print("optical flow:", of_l[0], of_r[0]," err from desired:",abs(of_l[0])-abs(of_r[0]),"y vel",v,"z vel",v2,"err compensated",abs(of_l[0])-abs(of_r[0])- \
            #    2*v*np.cos(math.radians(self.roll_c)) - 2*v2*np.sin(math.radians(self.roll_c))," roll:",self.roll_c)
            #self.alt_c_auto +=  0.05*(of - 0.5 - v)
        if mode==3:
            turn = self.get_imu()
            v = turn[5]
            ...
            of_r = of[1]
            self.alt_c = 10.0
            self.yaw_c = 5*(abs(of_l[0])-abs(of_r[0]))
            self.vy_c = self.yaw_c/45
            self.speed_val = 0.5
            # print("optical flow:", of_l[0], of_r[0]," err from desired:",abs(of_l[0])-abs(of_r[0]),"turning vel",v,"err compensated",abs(of_l[0])-abs(of_r[0])-2*v," yaw:",self.yaw_c)
            #self.alt_c_auto +=  0.05*(of - 0.5 - v)


        # def automatic_control_send(self):
        self.compute_velocity_control()
        self.set_command(self.roll_c, self.pitch_c, self.yawrate_c, self.alt_c_auto)
        # return of-
